...
namespace
{
// XFeat路径: 每个MapPoint保留固定数量的描述子原型。
constexpr int kXFeatDescriptorBankSize = 4;
    ...
    mnFound(1), mbBad(false), mpReplaced(NULL), mpMap(pMap), mnOriginMapId(pMap->GetId())
{
    ...
    if(IsFloatDescriptorType(mDescriptor))
    {
        // XFeat路径: 新建点先用首个观测初始化描述子库。
        mXFeatDescriptorBank.clear();
        mXFeatDescriptorBank.push_back(mDescriptor.clone());
    }
...
void MapPoint::ComputeDistinctiveDescriptors()
{
    ...
    for(size_t i=0;i<N;i++)
    {
        ...
        {
            const float distij = DescriptorDistanceAdaptive(vDescriptors[i],vDescriptors[j]);
    ...
    for(size_t i=0;i<N;i++)
    {
        ...
        const float median = vDists[(N-1)/2];
    ...
    std::vector<cv::Mat> xfeatBank;
    if(IsFloatDescriptorType(vDescriptors[BestIdx]))
    {
        // XFeat路径: 从观测描述子中构建多原型库。
        // 先保留medoid，再贪心加入与已选原型最不相似的样本，覆盖更多视角变化。
        std::vector<int> selected;
        selected.reserve(std::min<int>(kXFeatDescriptorBankSize, static_cast<int>(N)));
        selected.push_back(BestIdx);

        while(static_cast<int>(selected.size()) < kXFeatDescriptorBankSize &&
              static_cast<int>(selected.size()) < static_cast<int>(N))
        {
            ...
            for(size_t i = 0; i < N; ++i)
            {
                ...
                float minDistToSelected = std::numeric_limits<float>::infinity();
                for(int idx : selected)
                {
                    minDistToSelected = std::min(minDistToSelected, Distances[i][idx]);
                }
        ...
        xfeatBank.reserve(selected.size());
        for(int idx : selected)
            xfeatBank.push_back(vDescriptors[idx].clone());
    }

    {
        unique_lock<mutex> lock(mMutexFeatures);
        mDescriptor = vDescriptors[BestIdx].clone();
        // XFeat路径: 仅float描述子时启用描述子库；ORB路径保持空库。
        mXFeatDescriptorBank = std::move(xfeatBank);
    }
}
...
std::vector<cv::Mat> MapPoint::GetXFeatDescriptorBank()
{
    unique_lock<mutex> lock(mMutexFeatures);
    std::vector<cv::Mat> out;
    out.reserve(mXFeatDescriptorBank.size());
...
{
    unique_lock<mutex> lock(mMutexFeatures);
    if(mXFeatDescriptorBank.empty() || query.empty())
        return mDescriptor.clone();

    float bestDist = std::numeric_limits<float>::infinity();
    int bestIdx = -1;
    for(size_t i = 0; i < mXFeatDescriptorBank.size(); ++i)
    {
        const cv::Mat& d = mXFeatDescriptorBank[i];
        if(d.empty())
            continue;
    ...
    if(bestIdx >= 0)
        return mXFeatDescriptorBank[bestIdx].clone();
    return mDescriptor.clone();
}
