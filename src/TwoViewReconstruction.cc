...
namespace ORB_SLAM3
{
                                             ...
                                             Sophus::SE3f &T21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
    {
        ...
        float RH = SH/(SH+SF);
        const int nInliersH = CountInliers(vbMatchesInliersH);
        const int nInliersF = CountInliers(vbMatchesInliersF);

        float minParallax = 1.0;
        bool bOK = false;
        bool bPrimarySuccess = false;
        bool bSecondaryTried = false;
        bool bSecondarySuccess = false;
        struct ModelDiag
        {
            int maxGood = -1;
            int minGood = -1;
        ...
        ModelDiag primaryDiag;
        ModelDiag secondaryDiag;

        const bool bPreferH = (RH > 0.50f); // if(RH > 0.40) in some ORB-SLAM versions
        const char primaryModel = bPreferH ? 'H' : 'F';
        const char secondaryModel = bPreferH ? 'F' : 'H';

        //调试: 主模型失败后自动尝试另一模型，减少“模型判别临界值”导致的初始化失败。
        auto tryReconstruct = [&](const char model,
                                  Sophus::SE3f& outT21,
                                  std::vector<cv::Point3f>& outP3D,
                                  std::vector<bool>& outTriangulated,
                                  ModelDiag& diag) -> bool
        {
            if(model == 'H')
                return ReconstructH(vbMatchesInliersH, H, mK, outT21, outP3D, outTriangulated, minParallax, 50,
                                    &diag.maxGood, &diag.competitor, &diag.minGood, &diag.parallax);
            return ReconstructF(vbMatchesInliersF, F, mK, outT21, outP3D, outTriangulated, minParallax, 50,
                                &diag.maxGood, &diag.minGood, &diag.competitor, &diag.parallax);
        };

        Sophus::SE3f T21Candidate;
        std::vector<cv::Point3f> vP3DCandidate;
        std::vector<bool> vbTriangulatedCandidate;

        bPrimarySuccess = tryReconstruct(primaryModel, T21Candidate, vP3DCandidate, vbTriangulatedCandidate, primaryDiag);
        if(bPrimarySuccess)
        {
            T21 = T21Candidate;
            vP3D = vP3DCandidate;
            vbTriangulated = vbTriangulatedCandidate;
            bOK = true;
        }
        else
        {
            bSecondaryTried = true;
            Sophus::SE3f T21Secondary;
            std::vector<cv::Point3f> vP3DSecondary;
            std::vector<bool> vbTriangulatedSecondary;
            bSecondarySuccess = tryReconstruct(secondaryModel, T21Secondary, vP3DSecondary, vbTriangulatedSecondary, secondaryDiag);
            if(bSecondarySuccess)
            {
                T21 = T21Secondary;
                vP3D = vP3DSecondary;
                vbTriangulated = vbTriangulatedSecondary;
                bOK = true;
            }
        }
        ...
        if(IsXFeatInitDebugEnabled())
        {
            std::cout << "[TwoViewReconstruction] "
                      ...
                      << " primary_max_good=" << primaryDiag.maxGood
                      << " primary_min_good=" << primaryDiag.minGood
                      << " primary_competitor=" << primaryDiag.competitor
                      << " primary_parallax=" << primaryDiag.parallax
                      << " primary_success=" << (bPrimarySuccess ? "true" : "false")
                      << " secondary_tried=" << (bSecondaryTried ? "true" : "false")
                      << " secondary_model=" << (bSecondaryTried ? std::string(1, secondaryModel) : "N")
                      << " secondary_max_good=" << secondaryDiag.maxGood
                      << " secondary_min_good=" << secondaryDiag.minGood
                      << " secondary_competitor=" << secondaryDiag.competitor
                      << " secondary_parallax=" << secondaryDiag.parallax
                      << " secondary_success=" << (bSecondarySuccess ? "true" : "false")
                      << " success=" << (bOK ? "true" : "false")
                      << std::endl;
        }
    ...
    bool TwoViewReconstruction::ReconstructF(vector<bool> &vbMatchesInliers, Eigen::Matrix3f &F21, Eigen::Matrix3f &K,
                                             ...
                                             int* pMaxGood, int* pMinGood, int* pNSimilar, float* pBestParallax)
    {
        int N=0;
        ...
        const float bestParallax = (maxGood == nGood1) ? parallax1 :
                                   (maxGood == nGood2) ? parallax2 :
                                   (maxGood == nGood3) ? parallax3 : parallax4;
        if(pMaxGood)
            *pMaxGood = maxGood;
        if(pMinGood)
            *pMinGood = nMinGood;
    ...
    bool TwoViewReconstruction::ReconstructH(vector<bool> &vbMatchesInliers, Eigen::Matrix3f &H21, Eigen::Matrix3f &K,
                                             Sophus::SE3f &T21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated,
                                             int* pBestGood, int* pSecondBestGood, int* pMinGood, float* pBestParallax)
    {
        int N=0;
        for(size_t i=0, iend = vbMatchesInliers.size() ; i<iend; i++)
            if(vbMatchesInliers[i])
        ...
        const int nMinGood = std::max(minTriangulated, static_cast<int>(0.9f * static_cast<float>(N)));
        if(pBestGood)
            *pBestGood = bestGood;
        if(pSecondBestGood)
            *pSecondBestGood = secondBestGood;
        if(pMinGood)
            *pMinGood = nMinGood;
        if(pBestParallax)
            *pBestParallax = bestParallax;

        if(secondBestGood<0.75*bestGood && bestParallax>=minParallax && bestGood>nMinGood)
        {
