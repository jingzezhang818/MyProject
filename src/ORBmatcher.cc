...
namespace ORB_SLAM3
{
    ...
    {
        // [XFEAT_FIX_20260414] XFeat descriptors are float vectors; use L2 NN matching instead of ORB BoW/Hamming.
        ...
        cv::Mat descKF, descF;
        if(pKF->mDescriptors.type() == CV_32F) descKF = pKF->mDescriptors;
        else pKF->mDescriptors.convertTo(descKF, CV_32F);

        if(F.mDescriptors.type() == CV_32F) descF = F.mDescriptors;
        else F.mDescriptors.convertTo(descF, CV_32F);

        cv::BFMatcher matcher(cv::NORM_L2, false);
        std::vector<std::vector<cv::DMatch>> knnMatches;
        matcher.knnMatch(descKF, descF, knnMatches, 2);

        int nmatches = 0;
        for(size_t i = 0; i < knnMatches.size(); ++i)
        {
            if(i >= vpMapPointsKF.size())
                continue;
            ...
            const std::vector<cv::DMatch>& m = knnMatches[i];
            if(m.empty())
                continue;
            ...
            if(m.size() > 1)
            {
                ...
                if(best.distance >= mfNNratio * second.distance)
                    continue;
            }

            const int bestIdxF = best.trainIdx;
            if(bestIdxF < 0 || bestIdxF >= F.N)
                continue;

            if(vpMapPointMatches[bestIdxF])
                continue;
        ...
        for(int i=0; i<LastFrame.N; i++)
        {
            ...
            if(pMP)
            {
                if(!LastFrame.mvbOutlier[i])
                {
                    ...
                    if(bestIdx2>=0 && bestDist<=TH_HIGH)
                    {
